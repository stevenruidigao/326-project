...
router.get(
    "/me", requiresAuth,
    (req, res) => { res.json(users.serialize(req.user, req.user._id)); });
...
router.put(
    ...
    asyncHandler(async (req, res, next) => {
      ...
      const validData = Object.entries(req.body).filter(
          ([ key ]) => users.VALID_KEYS.includes(key),
      );
      ...
      for (const key of ["known", "interests"]) {
        ...
        data[key] =
            Array.isArray(value)
                ? value
                : String(value).split(/,\s*/g).filter((str) => str !== "");
      }
    ...
    (req, res, next) => {
      ...
      form.parse(req, (err, fields, files) => {
        ...
        return (file ? readFile(file.filepath) : Promise.resolve())
            .then((buffer) =>
                      users.updateAvatar(req.user, file?.mimetype, buffer))
            .then((updatedUser) => {
